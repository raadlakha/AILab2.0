...
> Role restrictions cap the maximum privilege level the skill inherits during execution — even if the invoking user has admin, the skill runs within defined bounds. This is a security best practice: skills should operate with least-privilege.
...
![NASK — Add Skill Input: incidentextendrecord](<../.gitbook/assets/NASK-generatewebsearchquestion-step3 (2).png>)
...
![NASK — Edit Prompt Name: Incident Summarization with pencil icon](../.gitbook/assets/NASK-generatewebsearchquestion-step17.png)
...
![NASK — Prompt Renamed to Generate Web Search Questions for Resolution Plan](../.gitbook/assets/NASK-generatewebsearchquestion-step18.png)
...
| Provider API | `Chat Completion` (auto-populated from the resource skill's provider)                                           |

> The **Resource** field references the published `CreateOptimalSearchQuery` skill (built in [doc 04a-now-assist-skill-kit-createoptimalsearchqueryskill.md](04a-now-assist-skill-kit-createoptimalsearchqueryskill.md)). A platform warning appears: _"Ensure that Access Control Lists (ACLs) are correctly configured for the resource so that the skill can access it."_ — verify that `CreateOptimalSearchQuery` has `itil and x_snc_apacaienable.incident_extend_user` as its role restriction and `Any authenticated user` access, matching the parent skill.
...
![Add skill as a tool — Summary](<../.gitbook/assets/NASK-generatewebsearchquestion-step10 (2).png>)
...
1. Open the file at [`../NASKprompts/GenerateWebSearchQnsForResolutionPlan-Prompt`](https://raw.githubusercontent.com/raadlakha/AILab2.0/main/NASKprompts/GenerateWebSearchQnsForResolutionPlan-Prompt) in the lab repository
...
![NASK — Run Test Dialog: Input and Security Controls](../.gitbook/assets/NASK-generatewebsearchquestion-step19.png)
...
![NASK — Test Prompt Response: Web Search Query](../.gitbook/assets/NASK-generatewebsearchquestion-step20.png)
...
| ---------------------------------------------------- | -------------------------------------------------------- | --------- |
| Now LLM Service (Now LLM Generic) — Default provider | `Generate Web Search Questions for Resolution Plan (v1)` | ✅ Checked |
...
Locate `GenerateWebSearchQnsForResolutionPlan` under the **Available** tab in the **Other** workflow.
...
| Tool condition        | None (Always run)                                           |
| Prompt                | `Generate Web Search Questions for Resolution Plan (v1)`    |
