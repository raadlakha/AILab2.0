...
7. Wait for the solution to reach **Solution Complete** at **100%** progress (this may take a while, continue on with the lab - you can proceed on with the next steps as you are waiting for the model training to complete)
...
![User Record — Alex Rai with Roles (2)](../.gitbook/assets/check-user-permissions-6.png)

6. Ensure **snc\_external** role is not assigned to Alex Rai as well. If it is, remove **snc\_external** role from Alex Rai's roles. A user can only be either snc\_internal or snc\_external. It cannot be both.

![Check for snc\_external role)](../.gitbook/assets/check-user-permissions-7.png)
